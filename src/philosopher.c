...
/*   Updated: 2022/03/28 20:30:36 by rbicanic         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_philo_data(t_table *table)
{
	...
	while (table->philos[i].philo_id >= 0)
	{
		...
		printf(RED "\tdead ptr %p: %d\n"RESET, table->philos[i].dead, *table->philos[i].dead);
		printf(MAGENTA "\tforks ptr: %p\n\n" RESET, table->philos[i].forks);
		i++;
	}
	printf("\n\n");
}
...
int	main(int argc, char *argv[])
{
	...
	if (!argument_are_valid(argc, argv))
		return (0);
	if (!initialize_table(&table, &argv[1]))
